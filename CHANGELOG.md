...
formatting and theatron's SemVer policy
...
- **`keryx::url::join_base_path(base_url, path) -> String`** —
  ...
  and internal-slash preservation. (PR #93)
- **`keryx::sse::SseStream::next_with_timeout(deadline) -> Result<Option<SseEvent>, Elapsed>`**
  ...
  (PR #94)
- **dashboards lifecycle registry metadata** — registry surface and
  refined metadata for the dashboard lifecycle. (PRs #2, #4)
- **mekhane**: reconciled the forge-archive desktop surface for
  muda-backed app menus, global hotkey event plumbing, and the
  `default_icon` PNG-to-`Icon` helper.
